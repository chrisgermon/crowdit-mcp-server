...
echo "7) Update Crowd IT DigitalOcean API token"
echo ""
...
case $OPTION in
    ...
    7)
        echo ""
        read -sp "Enter Crowd IT DigitalOcean API Token: " CROWDIT_DO_TOKEN
        echo ""

        if [ -z "$CROWDIT_DO_TOKEN" ]; then
            echo "❌ API Token is required."
            exit 1
        fi

        # Create secret if it doesn't exist, then add version
        gcloud secrets describe CROWDIT_DIGITALOCEAN_TOKEN --project=$PROJECT_ID 2>/dev/null || \
            gcloud secrets create CROWDIT_DIGITALOCEAN_TOKEN --project=$PROJECT_ID --replication-policy=automatic

        echo -n "$CROWDIT_DO_TOKEN" | gcloud secrets versions add CROWDIT_DIGITALOCEAN_TOKEN --project=$PROJECT_ID --data-file=-
        echo "   ✓ Updated CROWDIT_DIGITALOCEAN_TOKEN"
        ;;
    *)
        echo "Invalid option"
        exit 1
